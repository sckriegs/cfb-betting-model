...
# Actual scores (pasted from previous command output)
csv_data = """
...
for _, row in merged.iterrows():
    ...
    # Get model pick
    # Calculate fair spread using V9 logic to replicate what pick-week did
    # Or just check fair_spread_home column
    fair_spread = row["fair_spread_home"]
    ...
    if model_pick == ats_result:
        ...
        wrong_picks.append({
            ...
            "Margin": home_margin,
            "Cover Margin": home_margin - (-market_spread),
            "Pick": model_pick,
            "Result": ats_result,
            "Fair Spread": fair_spread,
            "Edge": edge,
            "ATS Prob": row["p_home_covers"]
        })
