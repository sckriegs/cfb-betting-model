...
for _, row in merged.iterrows():
    ...
    fav_covered = False
    if spread < 0: # Home Fav
        if home_margin > -spread:
            fav_covered = True
    elif spread > 0: # Away Fav
        # Away margin > spread
    ...
    if home_margin == -spread: continue # Push
    
    total += 1
    if fav_covered:
        fav_covers += 1

print(f"Total Games with Spread: {total}")
print(f"Favorites Covered: {fav_covers}")
print(f"Favorite Cover Rate: {fav_covers/total:.1%}")
